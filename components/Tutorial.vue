...
export default {
  data(){
   ...
   created(){
    this.data = this.teste();
  },
  methods:{
    async teste(){
         const dados = await this.$axios.$get('/rua')
        // const dados = await this.$axios.$get('/company')
        // console.log(dados)

      // const {data} = await this.$axios.$get('https://significado.herokuapp.com/rua')
      return dados
    }
  }
